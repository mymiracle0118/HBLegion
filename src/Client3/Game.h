...
#define MAXCLIENTS			200 //added for bisle
...
{
public:
	char m_cSavedUsername[12], m_cSavedPassword[12];
	//cont code
	void NotifyMsg_ContPoints(char * pData);
	//int m_iPurchaseID;
	int m_iAre;
	int m_iElv;
	int m_iPing;
	int m_iPlayerTitle;
	int m_iTitleCount;
	int m_iTitleList[MAXTITLES];
	int m_iTitleUsesList[MAXTITLES];
	int m_iTitleMe;
	void NotifyMsg_TitleList(char*pData);
	//int m_iMobKill;
	//bisle event minimap
	int BisleEnemies[MAXCLIENTS][1];
	int BisleEnemyCount;
	char m_cBisleGuild[5][21];
	int m_iBisleGuild[5];
	int QuestListDialog;
	int QuestIDs[200];
	...
	void DrawSoccerInfo();
	BOOL bSetSprRangeObj(short sObjSpr, bool bUpdate = false);
	char m_cObjSprRange;
	BOOL m_pingSound, m_TopBar;
	...
	BOOL _bDecodeLogData();
	BOOL _bDecodeItemConfigFileContents();
	BOOL _bDecodeBuildItemFileContents();
	char CheckHeroItemEquipped();
	void CalcTotalDamage(int *iMinDmg, int *iMaxDmg, char cType, char cAttackType, short weaponStatus, bool bBankList = false);
	void CalcTotalItemEffect();
	void DrawDialogBox_QuestList();
	void DlgBoxClick_QuestList();
	void DrawDialogBox_AuctionHouse();
	void DlgBoxClick_AuctionHouse();
	void DrawDialogBox_SubAuctionHouse();
	void DlgBoxClick_SubAuctionHouse();
	void bItemDrop_SubAuctionHouse();
	void DlgBoxClick_AddCharges();
	void DrawDialogBox_AddCharges();
	void bItemDrop_AddCharges();
	//int iChargeVal, iChargeTotal, m_iType, iContVal;
	short m_sAddCharge;
	...
	void NotifyMsg_PingPlayers(char*pData);
	BOOL m_bHappyHour;
	...
	void DrawDialogBox_ChangeStats();
	void DlgBoxClick_ChangeStats();
	...
	void ReadSettings();
	void ReadPlayerSettings(char * cPlayerName);
	uint32 ReadSettingsVar(const char * var, char * cPlayerName);
	void WriteSettings();
	void WritePlayerSettings(char * cPlayerName);
	void WriteSettingsVar(const char * var, uint32 val, char * cPlayerName);
	...
	int GetBisleEnemy(char * pName, char* pGuildname);
   ...
   void DlgBoxClick_GuildContribute();//58
	...
	void DrawDialogBox_SlayerMenu();
	void DlgBoxClick_SlayerMenu();
	...
	void NotifyMsg_BisleEventToggle(char * pData);
	void NotifyMsg_BisleEventScoreboard(char * pData);
	...
	int _iGetFOE(int iStatus, bool bSetGlobal = false);
	void NoticementHandler(char * pData);
	void GetItemName(char * cItemName, DWORD dwAttribute, DWORD dwTempAttribute, char *pStr1, char *pStr2, char *pStr3);
	void GetItemName(CItem * pItem, char * pStr1, char * pStr2, char * pStr3);
	Vector<string> * GetItemName(char * cItemName, DWORD attr, DWORD tempattr, uint8 sockets[MAXITEMSOCKETS], uint32 count);
	Vector<string> * GetItemName(CItem * pItem, bool bBankList = false);
	...
	void MakeEffectLegionSprite( char* FileName, short sStart, short sCount, bool bAlphaEffect = TRUE);
	...
	void DrawNpcName(short sX, short sY, short sOwnerType, int iStatus, int iHP, int iMaxHP);
	...
	void EnableChat(chatTypes t, bool enable);
	...
	void PutAlignedString(int iX1, int iX2, int iY, char const * const pString, bool wordWrap, COLORREF color = RGB(0,0,0));
	...
	BOOL bSendCommand(DWORD dwMsgID, WORD wCommand, char cDir, int iV1, int iV2, int iV3, char const * const pString, int iV4 = NULL); 
	...
	void DebugLog(char * cStr, bool bTimeStamp = true);
	...
	void Glare(int iWeaponColor, int *iWeaponGlare);
	void DrawDruncncity();
	...
	char m_cPartyLeaderName[11];

	struct guildMember{
		short sX, sY;
		string cName;
		guildMember(char * inName) : cName(inName), sX(0), sY(0) {} 
	};
	std::vector<guildMember *> m_stGuildMember;
	typedef std::vector<guildMember *>::iterator guildIterator;
	...
	class CItem   * m_pItemConfigList[MAXITEMTYPES];
	class CItem   * m_pItemList[MAXITEMS];
	class CItem   * m_pBankList[MAXBANKITEMS];
	class CItem   * m_pAuctionList[MAXAUCTIONITEMS];
	ItemMap m_guildBankMap;
	uint32 m_guildBankVer;
	class CMagic * m_pMagicConfigList[MAXMAGICTYPE];
	...
	DWORD m_dwCommandTime, m_dwLastPingTime; //v2.15 SpeedHack
	...
	BOOL m_bIsPartyPingEnabled, m_bIsPartyLeader;
	...
	uint16 m_astoriaShieldsHP[MAXSIDES];
	...
	int m_iPlayerApprColor1, m_iPlayerApprColor2;
	...
	int m_iLevel, m_iContPts, m_iContribution;
	int m_stat[6];
...
//	int m_iAccountStatus;
	...
	int m_iApprColor1_IE, m_iApprColor2_IE;
	...
	WORD m_wR[27], m_wG[27], m_wB[27];
	WORD m_wWR[27], m_wWG[27], m_wWB[27];
	...
	char m_cBidAmountString[12], m_cBuyoutAmountString[12];
	char m_cLogOutCount;
	...
	int m_iMaxAP_SM;
	int m_iMaxAP_L;
	int m_iSpecialWeaponEffectType, m_iSpecialWeaponEffectValue;
