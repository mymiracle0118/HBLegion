...
CMapData::CMapData(class CGame * pGame)
{
	...
	{	m_stFrame[i][OBJECTSTOP].m_sMaxFrame		= 14;
		...
		m_stFrame[i][OBJECTRUN].m_sFrameTime		= 41/SPEEDHAX_RUN;
	...
	m_stFrame[65][OBJECTDYING].m_sMaxFrame		= 7 +3;
   	...
   	// Wyvern
	m_stFrame[66][OBJECTSTOP].m_sFrameTime		= 250;
	m_stFrame[66][OBJECTSTOP].m_sMaxFrame		= 7;
	m_stFrame[66][OBJECTMOVE].m_sFrameTime		= 90;
	m_stFrame[66][OBJECTATTACK].m_sFrameTime	= 100;
	m_stFrame[66][OBJECTATTACK].m_sMaxFrame		= 7;
	m_stFrame[66][OBJECTDAMAGE].m_sFrameTime	= 100;
	m_stFrame[66][OBJECTDAMAGE].m_sMaxFrame		= 7;
	m_stFrame[66][OBJECTDYING].m_sFrameTime		= 150;
...
BOOL __fastcall CMapData::bSetOwner(WORD wObjectID, int sX, int sY, int sType, int cDir, int iHP, int iMaxHP, short sAppr1, short sAppr2, short sAppr3, short sAppr4, int iApprColor1, int iApprColor2, int iStatus, char * pName, short sAction, short sV1, short sV2, short sV3, int iPreLoc, int iFrame)
{int   iX, iY, dX, dY;
 int   iChatIndex, iAdd;
 char  cTmpName[12];
 DWORD dwTime;
		...
		{	iX = m_iObjectIDcacheLocX[wObjectID] - m_sPivotX;
			...
			{	dX = iX;
				...
				{	sType      = m_pData[iX][iY].m_sOwnerType;
					...
					iApprColor1 = m_pData[iX][iY].m_iApprColor1;
					iApprColor2 = m_pData[iX][iY].m_iApprColor2;
					iStatus    = m_pData[iX][iY].m_iStatus;
					iEffectType  = m_pData[iX][iY].m_iEffectType;
					iEffectFrame = m_pData[iX][iY].m_iEffectFrame;
					iEffectTotalFrame = m_pData[iX][iY].m_iEffectTotalFrame;
					iHP = m_pData[iX][iY].m_iHP;
					iMaxHP = m_pData[iX][iY].m_iMaxHP;
				}
				ZeroMemory(cTmpName, sizeof(cTmpName));
		...
		{	iX = abs(m_iObjectIDcacheLocX[wObjectID]) - m_sPivotX;
			...
			{	dX = iX;
				...
				if (sAction != OBJECTNULLACTION) {
					...
					iApprColor1 = m_pData[iX][iY].m_iDeadApprColor1;
					iApprColor2 = m_pData[iX][iY].m_iDeadApprColor2;
...
EXIT_SEARCH_LOOP:;
	...
	{	m_pData[dX][dY].m_wDeadObjectID        = m_pData[dX][dY].m_wObjectID;
		...
		m_pData[dX][dY].m_iDeadApprColor1       = m_pData[dX][dY].m_iApprColor1;
		m_pData[dX][dY].m_iDeadApprColor2       = m_pData[dX][dY].m_iApprColor2;
		...
		m_pData[dX][dY].m_iHP = m_pData[dX][dY].m_iMaxHP = 0;
	...
	{	m_pData[dX][dY].m_wObjectID   = wObjectID;
		...
		m_pData[dX][dY].m_iHP  = iHP;
		m_pData[dX][dY].m_iMaxHP = iMaxHP;
		m_pData[dX][dY].m_sAppr1      = sAppr1;
		m_pData[dX][dY].m_sAppr2      = sAppr2;
		m_pData[dX][dY].m_sAppr3      = sAppr3;
		m_pData[dX][dY].m_sAppr4      = sAppr4;
		m_pData[dX][dY].m_iApprColor1  = iApprColor1;
		m_pData[dX][dY].m_iApprColor2  = iApprColor2;
	...
	{	m_pData[dX][dY].m_wDeadObjectID   = wObjectID;
		...
		m_pData[dX][dY].m_iDeadApprColor1  = iApprColor1;
		m_pData[dX][dY].m_iDeadApprColor2  = iApprColor2;
...
BOOL __fastcall CMapData::bGetOwner(short sX, short sY, short * pOwnerType, char * pDir, short * pAppr1, short * pAppr2, short * pAppr3, short * pAppr4, int * pApprColor1, int * pApprColor2, int * pStatus, char * pName, char * pAction, char * pFrame, int * pChatIndex, short * pV1, short * pV2)
{
 int dX, dY;
	...
	*pApprColor1 = m_pData[dX][dY].m_iApprColor1;
	*pApprColor2 = m_pData[dX][dY].m_iApprColor2;
...
/*BOOL __fastcall CMapData::bGetDeadOwner(short sX, short sY, short * pOwnerType, char * pDir, short * pAppr1, short * pAppr2, short * pAppr3, short * pAppr4, int * pApprColor1, int * pApprColor2, char * pFrame, char * pName, short * pItemSprite, short * pItemSpriteFrame, DWORD * pItemAttribute, char * pItemName, int * pChatIndex)
{
 int dX, dY;
	...
	*pApprColor1 = m_pData[dX][dY].m_iDeadApprColor1;
	*pApprColor2 = m_pData[dX][dY].m_iDeadApprColor2;
	...
	*pItemAttribute = m_pData[dX][dY].m_dwItemAttribute;
	strcpy((char *)pItemName, m_pData[dX][dY].m_cItemName);
					...
					if ( m_pData[dX][dY].m_cOwnerAction == OBJECTSTOP ) {
						...
						case 54: // Dark-Elf
							if ((m_pData[dX][dY].m_cOwnerFrame == 1) || (m_pData[dX][dY].m_cOwnerFrame == 5)) {
								m_pGame->PlaySound('C', 8, sDist/2, lPan);
								if (((m_pData[dX][dY].m_sAppr4 & 0x000F) != 0) && ((m_pData[dX][dY].m_iStatus & 0x10) == 0)) {
									cTotalFrame = 8;
						...
						case 6:
							if (m_pData[dX][dY].m_cOwnerFrame == 2) {
								...
								}else if (((sWeaponType >= 20) && (sWeaponType <= 39)) || sWeaponType == 45) {
						...
						case 6:
							if ((m_pData[dX][dY].m_sV3 >= 20) && (m_pData[dX][dY].m_cOwnerFrame == 2)) {
								...
								if  ( m_pGame->bHasHeroSet(m_pData[dX][dY].m_sAppr3, m_pData[dX][dY].m_sAppr4, m_pData[dX][dY].m_sOwnerType) == 3)
									m_pGame->bAddNewEffect(243, m_sPivotX + dX, m_sPivotY + dY, m_sPivotX + dX , m_sPivotY + dY, 0, 1);
...
BOOL CMapData::bSetItem(short sX, short sY, short sIDnum, char cItemColor, DWORD dwItemAttribute, BOOL bDropEffect)
{int dX, dY;
 int sAbsX, sAbsY, sDist;
	...
	m_pData[dX][dY].m_sIDnum = sIDnum;
	/*m_pData[dX][dY].m_sItemSprite      = sItemSpr;
	m_pData[dX][dY].m_sItemSpriteFrame = sItemSprFrame;*/
	m_pData[dX][dY].m_dwItemAttribute = dwItemAttribute;
	//strcpy(m_pData[dX][dY].m_cItemName, pItemName);
...
BOOL __fastcall CMapData::bSetDeadOwner(WORD wObjectID, short sX, short sY, short sType, char cDir, short sAppr1, short sAppr2, short sAppr3, short sAppr4, int iApprColor1, int iApprColor2, int iStatus, char * pName)
{
 int  dX, dY;
 char pTmpName[12];
 BOOL bEraseFlag = FALSE;
	...
	m_pData[dX][dY].m_iDeadStatus     = iStatus;
	m_pData[dX][dY].m_cDeadOwnerFrame = -1;
	m_pData[dX][dY].m_iHP = 0;
	m_pData[dX][dY].m_iMaxHP = 0;
...
void CMapData::GetOwnerStatusByObjectID(WORD wObjectID, char *pOwnerType, char *pDir, short *pAppr1, short *pAppr2, short *pAppr3, short *pAppr4, int * pStatus, int * pColor1, int * pColor2, char * pName)
{ int iX, iY;
	for (iX = 0; iX < MAPDATASIZEX; iX++)
	for (iY = 0; iY < MAPDATASIZEY; iY++)
	if (m_pData[iX][iY].m_wObjectID == wObjectID)
	{	*pOwnerType = (char)m_pData[iX][iY].m_sOwnerType;
		...
		*pColor1     = m_pData[iX][iY].m_iApprColor1;
		*pColor2     = m_pData[iX][iY].m_iApprColor2;
