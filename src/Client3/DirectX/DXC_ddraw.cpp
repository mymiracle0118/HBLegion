...
BOOL DXC_ddraw::bInit(HWND hWnd)
{
 ...
 int            iS, iD;
	...
	m_sBackB4Pitch       = (short)ddsd.lPitch / (bitsPerPixel == 32 ? 4 : 2);
...
void DXC_ddraw::SetFontStyle(char cType)
{
	m_hFontInUse = NULL;

	switch(cType) {
	case 0:
		m_hFontInUse = CreateFont(16,0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH, "Trebuchet MS");
		break;
	case 1:
		m_hFontInUse = CreateFont(16,0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH, "Comic Sans MS");
		break;
	default:
		m_hFontInUse = CreateFont(16,0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH, "Trebuchet MS");
		break;
...
void DXC_ddraw::ChangeDisplayMode(HWND hWnd)
{
	...
	else
	{
		...
		ddVal = m_lpDD4->SetDisplayMode(640, 480, bitsPerPixel,0,0);
		if (ddVal != DD_OK) return;
		memset( (VOID *)&ddsd, 0, sizeof(ddsd) );
		ddsd.dwSize = sizeof( ddsd );
		ddsd.dwFlags = DDSD_CAPS | DDSD_BACKBUFFERCOUNT;
...
void DXC_ddraw::_TestPixelFormat()
{
 ...
 HRESULT       hResult;
	...
	if (hResult == DD_OK)
	{
	   ...
	   else if (ddSurfaceDesc2.ddpfPixelFormat.dwRBitMask == 0x00007C00) {
		   m_cPixelFormat = 2;
		  // RGB 5:5:5 
	   }
	   else if (ddSurfaceDesc2.ddpfPixelFormat.dwRBitMask == 0x0000001F) {
		   m_cPixelFormat = 3;
		  // BGR 5:6:5 
	   }
	   else if (ddSurfaceDesc2.ddpfPixelFormat.dwRBitMask == 0x00FF0000) {
		   m_cPixelFormat = 4;
		  // ARGB 8:8:8:8
	   }
	}
...
void DXC_ddraw::DrawShadowBox(short sX, short sY, short dX, short dY, int iType)
{
	...
	if (iType == 0) {
		...
		case 4:
			dst32 = (uint32*)m_pBackB4Addr + sX + ((sY)*(m_sBackB4Pitch));

			for (iy = 0; iy <= (dY - sY); iy++) {
				for (ix = 0; ix <= (dX - sX); ix++) 
					dst32[ix] = (dst32[ix] & 0xfcfcfc) >> 2;
				
				dst32 += m_sBackB4Pitch;
			}
			break;
		}
	}
...
void DXC_ddraw::DrawText(LPRECT pRect, char const * const pString, COLORREF rgb, bool wordWrap)
{
	SetTextColor(m_hDC, rgb);
	::DrawText(m_hDC, pString, strlen(pString), pRect, DT_CENTER | ((wordWrap) ? DT_WORDBREAK : 0) |
		DT_NOCLIP | DT_NOPREFIX);//v2.15
}

void DXC_ddraw::DrawTextEx(LPRECT pRect, char const * const pString, COLORREF rgb, DWORD flags, bool wordWrap)
{
	SetTextColor(m_hDC, rgb);
	::DrawText(m_hDC, pString, strlen(pString), pRect, DT_NOCLIP | ((wordWrap) ? DT_WORDBREAK : 0) | 
		DT_NOPREFIX | flags);//v2.15
}
