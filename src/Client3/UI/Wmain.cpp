...
{
	LPWSTR lpszCmdLine, *lpszArgv;
	int iNumArgc = NULL;
	char cTemp[128], cCommandLine[128];

	lpszCmdLine = GetCommandLineW();

	lpszArgv = CommandLineToArgvW(lpszCmdLine, &iNumArgc);
	
	ZeroMemory(cTemp, sizeof(cTemp));
	ZeroMemory(cCommandLine, sizeof(cCommandLine));
	
	for(int i = 0; i < iNumArgc; i++)
	{	wcstombs(cTemp, lpszArgv[i], wcslen(lpszArgv[i])+1);
		strcat(cCommandLine, cTemp);
		if (i < iNumArgc-1) strcat(cCommandLine, " ");
	}
	GlobalFree(lpszArgv);
	/*
	if (strcmp(cCommandLine, "_LK!(-*\\") != 0) {
		MessageBox(NULL, "Please execute the game client with the launcher.\nIf this doesn't work, download the new launcher and try again.\nYou can find the new launcher at http://helbreathlegion.net/download.php", "Helbreath Legion", MB_ICONEXCLAMATION | MB_OK);
		return 0;
	}*/
    ...
    if (!GetProcessAffinityMask(process, &processAffinityMask, &systemAffinityMask)) return 0;
	...
	if (!success)
	{	MessageBox(NULL, "Cannot mask process affinity!", "ERROR!", MB_OK);
		return 0;
	}
...
{	int cx = GetSystemMetrics(SM_CXFULLSCREEN)/2;
	...
	G_hWnd = CreateWindowEx(NULL, szAppClass, winName, dwStyle, cx-320, cy-240, 
