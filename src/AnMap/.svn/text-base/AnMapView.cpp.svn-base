...
void CAnMapView::RenderTile()
{
	...
	if( m_pDoc->m_pSprite[m_pDoc->m_iBrushIndex] == NULL ) 
		return;
	...
	for( iy = cy ; iy < m_ptClipping.y+cy ; iy++ )
	{
		for( ix = cx ; ix < m_ptClipping.x+cx ; ix++ )
		{
			...
			if( sSpr < 0 || sSpr >= DEF_MAXSPRITES ) 
				continue;
			if( m_pDoc->m_pSprite[sSpr] == NULL ) 
				continue;
	...
	if( m_pDoc->m_cWorkMode > 0 )
	{
		for( iy = cy ; iy < m_ptClipping.y+cy ; iy++ )
		{
			for( ix = cx ; ix < m_ptClipping.x+cx ; ix++ )
			{
				...
				if( m_pDoc->m_tile[ix][iy].m_canBuild ) BufferDC.Draw3dRect( ((ix-cx)<<5)+2, ((iy-cy)<<5)+2, DEF_TILESIZE-4, DEF_TILESIZE-4, RGB(50,255,255), RGB(50,255,255) );
			}
		}
		if( m_pDoc->m_cWorkMode == 5 )
		{
			for( iy = cy ; iy < m_ptClipping.y+cy ; iy++ )
			{
				for( ix = cx ; ix < m_ptClipping.x+cx ; ix++ )
		...
		if( m_pDoc->m_cWorkMode == 4 ) BufferDC.Draw3dRect( (m_pDoc->m_mousept.x<<5), (m_pDoc->m_mousept.y<<5), DEF_TILESIZE, DEF_TILESIZE, RGB(150, 255, 255), RGB(150, 255, 255) );
		if( m_pDoc->m_cWorkMode == 5 ) BufferDC.Draw3dRect( (m_pDoc->m_mousept.x<<5), (m_pDoc->m_mousept.y<<5), DEF_TILESIZE, DEF_TILESIZE, RGB(150, 255, 150), RGB(150, 255, 150) );
	}
	...
	cTmp.Format( "Tile : %d", m_pDoc->m_tile[m_pDoc->m_ScrollPt.x+m_pDoc->m_mousept.x][m_pDoc->m_ScrollPt.y+m_pDoc->m_mousept.y].m_sTileSprite );
	SetStatusText(4, cTmp);
	cTmp.Format( "Tile Frame : %d", m_pDoc->m_tile[m_pDoc->m_ScrollPt.x+m_pDoc->m_mousept.x][m_pDoc->m_ScrollPt.y+m_pDoc->m_mousept.y].m_sTileSpriteFrame );
	SetStatusText(5, cTmp);
