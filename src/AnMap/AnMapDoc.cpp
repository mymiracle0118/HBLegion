...
BEGIN_MESSAGE_MAP(CAnMapDoc, CDocument)
	...
	ON_COMMAND(ID_TOOL_FIX_BLANK, OnFixBlank)
...
void CAnMapDoc::SetRightView( CAnMapView* pView )
{
	...
	MakeSprite( "structures1",		50, 20);
	MakeSprite( "Sinside1",			70, 27 );
	MakeSprite( "trees1",			100, 46 );
	MakeSprite( "objects1",			200, 10 );
	MakeSprite( "objects2",			211, 5 );
	MakeSprite( "objects3",			216, 4 );
	MakeSprite( "objects4",			220, 2 );
	MakeSprite( "Tile223-225",		223, 3 );
	MakeSprite( "Tile226-229",		226, 4 );
	MakeSprite( "objects5",			230, 9 );
	MakeSprite( "objects6",			238, 4);
	MakeSprite( "objects7",			242, 7);
	MakeSprite( "objects8",			249, 1);
	MakeSprite( "maptiles2",		300, 15 );
	MakeSprite( "maptiles3",		315, 5);
	...
	MakeSprite( "Tile462-473",	462, 12);
	MakeSprite( "Tile474-478",	474, 5);
	MakeSprite( "Tile479-488",	479, 10);
	MakeSprite( "Tile489-522",	489, 34);	// Diuuude Drunken City
	MakeSprite( "Tile523-530",	523, 8);	// Diuuude Rampart
	MakeSprite( "Tile531-540",	531, 10);	// Diuuude GodH + Pont
	MakeSprite( "Tile541-545",	541, 5);	// Diuuude GodH
...
#ifdef _DEBUG
		char cTxt[128];
		sprintf(cTxt, "Tile Not Found In %s", FileName);
		MessageBox( NULL, cTxt, NULL, MB_OK );
#endif
		return;
	}
	...
	case 77: //'M'
		{
			...
			if( m_bWorkMode )
			{
				if( m_cWorkMode > 5 ) m_cWorkMode = 0;
				m_edit1.x = m_edit1.y = m_edit2.x = m_edit2.y = -1;
			}
			else
			{
	...
	for( int y=0 ; y<m_sMapDataSizeY ; y++ )
	{
		for( int x=0 ; x<m_sMapDataSizeX ; x++ )
		{
			...
			*sp = m_tile[x][y].m_sObjectSpriteFrame;
			...
			if( m_tile[x][y].m_bIsTeleport ) *cp = 0x40;
			else if( m_tile[x][y].m_bIsFarming ) *cp = 0x20;
			else if( m_tile[x][y].m_canBuild ) *cp = 0x10;
			cp += 2;
		}
	}
...
BOOL CAnMapDoc::bOpenMapDataFile(CString cFn)
{
	...
	for (int y=0 ; y<m_sMapDataSizeY ; y++ )
	{
		for(int x=0 ; x<m_sMapDataSizeX ; x++ )
		{
			...
			if (((*cp) & 0x10) != 0)
				m_tile[x][y].m_canBuild = TRUE;
			else m_tile[x][y].m_canBuild = FALSE;
			cp += 2;
		}
	}
	delete[] cpMapData;
			...
			if( m_pSprite[m_iBrushIndex]->IsBlankTile( sFrame ) == FALSE )
			{
				for( i = 0 ; i < DEF_MAX_MEMTILE ; i++ )
				{
					if( (pUndo->m_pUndoTile[i] == NULL) && (pUndo->m_pUndoTile[i] == NULL) )
					{
						...
						m_tile[x+ix][y+iy].m_canBuild = FALSE;
...
void CAnMapDoc::PutObject( BOOL bMakeUndo )
{
	...
	for( i = 0 ; i < DEF_MAX_MEMTILE ; i++ )
	{
		if( (pUndo->m_pUndoTile[i] == NULL) && (pUndo->m_pUndoTile[i] == NULL) )
		{
			pUndo->m_pUndoTile[i] = new class CTile(x, y);
			CopyTile( &(pUndo->m_pUndoTile[i]->tile), &(m_tile[x][y]) );
			if(m_tile[x][y].m_sObjectSpriteFrame)
			{
				m_tile[x][y].m_sObjectSpriteFrame = 0;
...
void CAnMapDoc::CopyTile( CTileSpr* pDstTile, CTileSpr* pSrcTile )
{
	...
	pDstTile->m_canBuild = pSrcTile->m_canBuild;
}
	...
	case 4:
		{
			...
			for( i = 0 ; i < DEF_MAX_MEMTILE ; i++ )
			{
				if( (pUndo->m_pUndoTile[i] == NULL) && (pUndo->m_pUndoTile[i] == NULL) )
				{
					pUndo->m_pUndoTile[i] = new class CTile(m_mousept.x+m_ScrollPt.x, m_mousept.y+m_ScrollPt.y);
					CopyTile( &(pUndo->m_pUndoTile[i]->tile), &(m_tile[m_mousept.x+m_ScrollPt.x][m_mousept.y+m_ScrollPt.y]) );
					m_tile[m_mousept.x+m_ScrollPt.x][m_mousept.y+m_ScrollPt.y].m_canBuild = FALSE;
					pUndo->m_pRedoTile[i] = new class CTile(m_mousept.x+m_ScrollPt.x, m_mousept.y+m_ScrollPt.y);
					CopyTile( &(pUndo->m_pRedoTile[i]->tile), &(m_tile[m_mousept.x+m_ScrollPt.x][m_mousept.y+m_ScrollPt.y]) );
					SetModify();
					break;
			...
			if( bLB || bRB )
			{
				...
				for( i = 0 ; i < DEF_MAX_MEMTILE ; i++ )
				{
					if( (pUndo->m_pUndoTile[i] == NULL) && (pUndo->m_pUndoTile[i] == NULL) )
					{
						pUndo->m_pUndoTile[i] = new class CTile(m_mousept.x+m_ScrollPt.x, m_mousept.y+m_ScrollPt.y);
						CopyTile( &(pUndo->m_pUndoTile[i]->tile), &(m_tile[m_mousept.x+m_ScrollPt.x][m_mousept.y+m_ScrollPt.y]) );
						if( bLB ) m_tile[m_mousept.x+m_ScrollPt.x][m_mousept.y+m_ScrollPt.y].m_canBuild = TRUE;
						else if( bRB ) m_tile[m_mousept.x+m_ScrollPt.x][m_mousept.y+m_ScrollPt.y].m_canBuild = FALSE;
						pUndo->m_pRedoTile[i] = new class CTile(m_mousept.x+m_ScrollPt.x, m_mousept.y+m_ScrollPt.y);
						CopyTile( &(pUndo->m_pRedoTile[i]->tile), &(m_tile[m_mousept.x+m_ScrollPt.x][m_mousept.y+m_ScrollPt.y]) );
						break;
					}
		...
		case 4:
			{
				...
				for( i = 0 ; i < DEF_MAX_MEMTILE ; i++ )
				{
					if( (pUndo->m_pUndoTile[i] == NULL) && (pUndo->m_pUndoTile[i] == NULL) )
					{
						pUndo->m_pUndoTile[i] = new class CTile(m_mousept.x+m_ScrollPt.x, m_mousept.y+m_ScrollPt.y);
						CopyTile( &(pUndo->m_pUndoTile[i]->tile), &(m_tile[m_mousept.x+m_ScrollPt.x][m_mousept.y+m_ScrollPt.y]) );
						m_tile[m_mousept.x+m_ScrollPt.x][m_mousept.y+m_ScrollPt.y].m_canBuild = TRUE;
						pUndo->m_pRedoTile[i] = new class CTile(m_mousept.x+m_ScrollPt.x, m_mousept.y+m_ScrollPt.y);
						CopyTile( &(pUndo->m_pRedoTile[i]->tile), &(m_tile[m_mousept.x+m_ScrollPt.x][m_mousept.y+m_ScrollPt.y]) );
						SetModify();
						break;
