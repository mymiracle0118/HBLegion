	...
	: CDialog(CProperties::IDD, pParent)
{
	...
	else
	{
		...
		BOOL bBuild = pDoc->m_tile[m_pt1.x][m_pt1.y].m_canBuild;
		for( iy=m_pt1.y ; iy<=m_pt2.y ; iy++ )
		{
			for( ix=m_pt1.x ; ix<=m_pt2.x ; ix++ )
			{
				...
				if( pDoc->m_tile[ix][iy].m_canBuild )
				{
					if( bBuild == FALSE ) m_bSameBuild = FALSE;
				}
				else
				{
					if( bBuild == TRUE ) m_bSameBuild = FALSE;
				}
			}
		}
	}
	...
	if( m_bSameBuild == FALSE )
	{
		m_iBuild = 2;
	}
	else
	{
		if( m_pDoc->m_tile[m_pt1.x][m_pt1.y].m_canBuild == TRUE ) m_iBuild = 1;
		else m_iBuild = 0;
	}
}
...
void CProperties::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_BUILD, m_iBuild);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CProperties, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_CHECK_BUILD, OnBnClickedCheckBuild)
END_MESSAGE_MAP()
...
void CProperties::OnOK() 
{
	...
	if( m_iBuild == 0 )
	{
		for( iy=m_pt1.y ; iy<=m_pt2.y ; iy++ )
		{
			for( ix=m_pt1.x ; ix<=m_pt2.x ; ix++ )
			{
				m_pDoc->m_tile[ix][iy].m_canBuild = FALSE;
			}
		}
	}
	else if( m_iBuild == 1 )
	{
		for( iy=m_pt1.y ; iy<=m_pt2.y ; iy++ )
		{
			for( ix=m_pt1.x ; ix<=m_pt2.x ; ix++ )
			{
				m_pDoc->m_tile[ix][iy].m_canBuild = TRUE;
			}
		}
	}
}
...
void CProperties::OnBnClickedCheckBuild()
{	
	m_iBuild++;
	if( m_bSameBuild )
	{		
		if( m_iBuild > 1 ) m_iBuild = 0;
	}
	else
	{
		if( m_iBuild > 2 ) m_iBuild = 0;
	}
	UpdateData( FALSE );
}
