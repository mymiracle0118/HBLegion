...
void PutLogFileList(char * cStr, char *FileName)
{
	...
	else if(!strstr(FileName, ".log"))
	{
		...
		else if(IsSame(FileName, CONNECTION_LOGFILE)){
			sprintf(Txt, "Connections %4d-%02d-%02d.log", SysTime.wYear, SysTime.wMonth, SysTime.wDay);
		}
    ...
    if(Server != NULL){
		for(WORD w = 0; w < MAXGAMESERVERS; w++){
			if(Server->GameServer[w] != NULL){
				...
				if(Server->GameServer[w]->IsBeingClosed || !Server->GameServer[w]->IsSocketConnected) SetTextColor(hdc, clMaroon);
		...
		case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
				...
				case IDOK:
					...
					UINT iResult = 0;
					mysql_init(&mySQL);
					if(!mysql_real_connect(&mySQL, Server->mySqlAddress, Server->mySqlUser, Server->mySqlPwd, "playerdb2", Server->mySqlPort, NULL, NULL)){
...
{
#ifndef _DEBUG
    hMutex = CreateMutex( NULL, FALSE, "HB Test LoginServer." ); // Create a Mutex 
