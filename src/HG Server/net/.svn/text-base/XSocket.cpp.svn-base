...
bool XSocket::bConnect(char * pAddr, int iPort, unsigned int uiMsg, bool addrIsUrl)
{
	...
	if(WSAAsyncSelect(m_Sock, m_hWnd, uiMsg, FD_CONNECT | FD_READ | FD_WRITE | FD_CLOSE) == SOCKET_ERROR) {
 ...
 char * pTemp;
	...
	while (m_pUnsentDataList[m_sHead] != NULL) {
		...
		if (iRet == m_iUnsentDataSize[m_sHead]) {
			...
			SafeCopy(pTemp, m_pUnsentDataList[m_sHead] + iRet, m_iUnsentDataSize[m_sHead] - iRet);
...
int XSocket::iSendMsg(char * cData, DWORD dwSize, char cKey)
{
 ...
 //char   m_msgBuff[50000], dataBuff[50000];
	...
	*wp = (WORD)dwSize + 3;
	...
	SafeCopy((char *)(m_pSndBuffer + 3), cData, dwSize);

   if (cKey != NULL) {//Encryption
		for (i = 0; i < dwSize; i++) {
			m_pSndBuffer[3+i] += (char)(i ^ cKey);
...
int XSocket::SendRawMsg(char * cData, DWORD dwSize)
{
	if (dwSize > m_dwBufferSize) return XSOCKEVENT_MSGSIZETOOLARGE;
	if (m_cType != XSOCK_NORMALSOCK) return XSOCKEVENT_SOCKETMISMATCH;
	if (m_cType == NULL) return XSOCKEVENT_NOTINITIALIZED;
...
//=============================================================================
bool XSocket::bAccept(XSocket * pXSock, unsigned int uiMsg)
{
 SOCKET			AcceptedSock;
 sockaddr		Addr;
 int	iLength;
 DWORD			dwOpt;
	...
	if(uiMsg == 0) return FALSE;
	
	if(WSAAsyncSelect(pXSock->m_Sock, m_hWnd, uiMsg, FD_READ | FD_WRITE | FD_CLOSE) == SOCKET_ERROR) {
		m_WSAErr = WSAGetLastError();
		return FALSE;
	}
...
int XSocket::iGetPeerAddress(char * pAddrString)
{
 ...
 int iRet, iLen;
	...
	SafeCopy(pAddrString, (char *)inet_ntoa(sockaddr.sin_addr));
