...
bool CheckResistingMagicSuccess(short sAttackerH, short sAttackerType, char cAttackerDir, short sTargetH, char cTargetType, int iHitRatio, char cMagicType)
{
	...
	case OWNERTYPE_PLAYER:
		...
		if (g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_NECK] != -1 && g_clientList[sTargetH]->m_pItemList[g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_NECK]]->m_sIDnum == ITEM_NECKLACEOFMEDUSA && cMagicType == MAGICTYPE_HOLDOBJECT)
		{
			if (dice(1,100) <= 75) return TRUE;
			return FALSE;
		}
	...
	if (sAttackerType == OWNERTYPE_NPC && cProtect == MAGICPROTECT_AMP && g_npcList[sAttackerH] && g_npcList[sAttackerH]->m_sType != 65) return TRUE;
	if (sAttackerType == OWNERTYPE_PLAYER && cProtect == MAGICPROTECT_AMP) return TRUE;

	if (iHitRatio < 1000 && (cProtect == MAGICPROTECT_PFM || cProtect == MAGICPROTECT_AMP)) return TRUE;
			...
			if (g_clientList[iClientH]->m_iSkillSSN[sSkillIndex] == 0) {
				if (g_clientList[iClientH]->m_sItemEquipmentStatus[ EQUIPPOS_TWOHAND ] != -1) {
					...
					if (g_clientList[iClientH]->m_pItemList[iWeaponIndex]->m_sRelatedSkill == sSkillIndex) {
						g_clientList[iClientH]->m_iPhysicalHitRatio++;
					}
				}
...
{
	int    iAP_SM, iAP_L, iBonusDmg, iAttackerHitRatio, iTargetDefenseRatio, iDestHitRatio, iResult, iAP_Abs_Armor, iAP_Abs_Shield;
	...
	case OWNERTYPE_PLAYER:
		...
		if (wWeaponType == 0) {
			...
			if (iAP_L  <= 0) iAP_L  = 1;
			iAttackerHitRatio = g_clientList[sAttackerH]->m_iPhysicalHitRatio + g_clientList[sAttackerH]->m_cSkillMastery[SKILL_HANDATTACK];

			g_clientList[sAttackerH]->m_sUsingWeaponSkill = SKILL_HANDATTACK;
			
			if( g_clientList[sAttackerH]->HasPartyRaidBonus() )
			{
				iAP_SM += 4;
				iAP_L  += 4;
			}
		}
		else if (((wWeaponType >= 1) && (wWeaponType < 40)) || (wWeaponType == 45)) {
			...
			iAttackerHitRatio = g_clientList[sAttackerH]->m_iPhysicalHitRatio;
		...
		/*if (g_clientList[sAttackerH]->m_heroArmourBonus == 1) {
			iAttackerHitRatio += 49*(1+((g_clientList[sAttackerH]->m_pItemList[g_clientList[sAttackerH]->m_sItemEquipmentStatus[EQUIPPOS_HEAD]]->m_dwAttribute & 0x0F000000) >> 24))/3;
			iAP_SM += 3+((g_clientList[sAttackerH]->m_pItemList[g_clientList[sAttackerH]->m_sItemEquipmentStatus[EQUIPPOS_HEAD]]->m_dwAttribute & 0x0F000000) >> 24);
			iAP_L += 3+((g_clientList[sAttackerH]->m_pItemList[g_clientList[sAttackerH]->m_sItemEquipmentStatus[EQUIPPOS_HEAD]]->m_dwAttribute & 0x0F000000) >> 24);
		}

		if (g_clientList[sAttackerH]->m_heroArmourBonus == 3) {
			iAttackerHitRatio += 36*(1+((g_clientList[sAttackerH]->m_pItemList[g_clientList[sAttackerH]->m_sItemEquipmentStatus[EQUIPPOS_HEAD]]->m_dwAttribute & 0x0F000000) >> 24))/3;
			iAP_SM += 2+((g_clientList[sAttackerH]->m_pItemList[g_clientList[sAttackerH]->m_sItemEquipmentStatus[EQUIPPOS_HEAD]]->m_dwAttribute & 0x0F000000) >> 24);
			iAP_L += 2+((g_clientList[sAttackerH]->m_pItemList[g_clientList[sAttackerH]->m_sItemEquipmentStatus[EQUIPPOS_HEAD]]->m_dwAttribute & 0x0F000000) >> 24);
		}*/
		...
		iAttackerHitRatio += g_clientList[sAttackerH]->m_iAddGlobalHitRatio;

		sAtkX = g_clientList[sAttackerH]->m_sX;
		sAtkY = g_clientList[sAttackerH]->m_sY;

		if(g_clientList[sAttackerH]->GetParty()) iPartyID = g_clientList[sAttackerH]->GetParty()->GetID();
		break;
	...
	case OWNERTYPE_NPC:
		...
		if(cTargetType == OWNERTYPE_PLAYER)
		{
			if(iAP_L >= 20 && iAP_SM >= 20){
				iAP_L -= g_clientList[sTargetH]->m_iSlayerAbsorb/5;
				iAP_SM -= g_clientList[sTargetH]->m_iSlayerAbsorb/5;
			}
		}
		iAttackerHitRatio = g_npcList[sAttackerH]->m_iPhysicalHitRatio;
	...
	case OWNERTYPE_PLAYER:
		...
		if (dwTime-g_clientList[sTargetH]->m_noAttackTime <= 15 _s) return 0;
		...
		if (g_clientList[sTargetH]->GetParty() && g_clientList[sTargetH]->GetParty()->GetID() == iPartyID)
			return 0;
		...
		if(cAttackerType == OWNERTYPE_NPC && g_clientList[sTargetH]->HasPartyHuntBonus())
		{
			iAP_SM *= 0.85;
			iAP_L  *= 0.85;
		}
	...
	case OWNERTYPE_NPC:
		...
		cTargetDir = g_npcList[sTargetH]->m_cDir;
		iTargetDefenseRatio = g_npcList[sTargetH]->m_iDefenseRatio/2;
	...
	if ( (cAttackerType == OWNERTYPE_PLAYER) && (cTargetType == OWNERTYPE_PLAYER) ) {
		...
		if((dwTime - g_clientList[sTargetH]->m_lastDamageTime) > 10000) {
	...
	if (wWeaponType >= 40 && wWeaponType != 45) {
	...
	if (cAttackerType == OWNERTYPE_PLAYER) {
		...
		if (cAttackerDir == cTargetDir && cTargetType == OWNERTYPE_PLAYER) {
			if (g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_BACK] != -1 && ((g_clientList[sTargetH]->m_pItemList[ g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_BACK] ]->m_dwAttribute & 0x0000F000) >> 12) == 8) {
				if (dice(1, 38) > ((g_clientList[sTargetH]->m_pItemList[ g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_BACK] ]->m_dwAttribute & 0x00000F00) >> 8)*3) iTargetDefenseRatio /= 2;
			}else iTargetDefenseRatio /= 2;
		}
	}
	else {
		switch (cProtect)
		...
		case MAGICPROTECT_PFA: 
			...
			case NPC_DARK_ELF: 
			case NPC_AGT:
				if(abs(sTgtX - g_npcList[sAttackerH]->m_sX) >= 1 || abs(sTgtY - g_npcList[sAttackerH]->m_sY) >= 1)
					return 0;
				break;
			}
			break;
	...
	if (cAttackerType == OWNERTYPE_PLAYER) {
		...
		if(cTargetType == OWNERTYPE_NPC)
		{	iAP_SM += g_clientList[sAttackerH]->m_iSlayerDamage;
			iAP_L += g_clientList[sAttackerH]->m_iSlayerDamage;
		}
	}
	...
	if (cAttackerType == OWNERTYPE_PLAYER) {
		...
		if (weaponStatus != -1 && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ] != NULL) {
			...
			if (cTargetType == OWNERTYPE_PLAYER && 
				(weapon == ITEM_KLONESSAXE || weapon == ITEM_KLONESSBLADE || weapon == ITEM_KLONESSESTERK || ITEM_KLONESSWAND_MS20 || ITEM_KLONESSWAND_MS10)) 
			{	
				iTemp = 0;
				for(short x = g_clientList[sAttackerH]->m_sX - 11; x <= g_clientList[sAttackerH]->m_sX + 11; x++)
				for(short y = g_clientList[sAttackerH]->m_sY - 9; y <= g_clientList[sAttackerH]->m_sY + 9; y++) {
					g_game->m_pMapList[ g_clientList[sAttackerH]->m_cMapIndex ]->GetOwner(&sOwner, &cOwnerType, x, y);
					if (cOwnerType == OWNERTYPE_PLAYER && g_clientList[sOwner] && g_clientList[sOwner]->m_side != g_clientList[sAttackerH]->m_side)
						iTemp += 2;
				}
		...
		if (cTargetType == OWNERTYPE_PLAYER) {
			weaponStatus = g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_NECK];
			if (cTargetType == OWNERTYPE_PLAYER && weaponStatus != -1 && g_clientList[sTargetH]->m_pItemList[ weaponStatus ] &&
				g_clientList[sTargetH]->m_pItemList[ weaponStatus ]->m_sIDnum == ITEM_NECKLACEOFKLONESS) 
			{	
				iTemp = 0;
				for(short x = g_clientList[sTargetH]->m_sX - 11; x <= g_clientList[sTargetH]->m_sX + 11; x++)
				for(short y = g_clientList[sTargetH]->m_sY - 9; y <= g_clientList[sTargetH]->m_sY + 9; y++) {
					g_game->m_pMapList[ g_clientList[sTargetH]->m_cMapIndex ]->GetOwner(&sOwner, &cOwnerType, x, y);
					if (cOwnerType == OWNERTYPE_PLAYER && g_clientList[sOwner] && g_clientList[sOwner]->m_side != g_clientList[sTargetH]->m_side)
						iTemp += 2;
				}
				if (iTemp > 10) iTemp = 10;
				iAP_SM -= iTemp;
	...
	if (iResult <= iDestHitRatio) {
		if (cAttackerType == OWNERTYPE_PLAYER) {
			...
			case ITEMSTAT_CRITICAL: 				
				if ((g_clientList[sAttackerH]->m_iSuperAttackLeft > 0) && (iAttackMode >= 20)) {
					iAP_SM += g_clientList[sAttackerH]->m_iAddCritDmg;
					iAP_L  += g_clientList[sAttackerH]->m_iAddCritDmg;
				}
				break;

			/*case ITEMSTAT_POISONING: // psn dmg calculation now used by m_iAddPsnDmg
				cAttackerSA = 61; 
		...
		case OWNERTYPE_PLAYER:
			...
			if ((dwTime - g_clientList[sTargetH]->m_dwTime) > RAGPROTECTIONTIME) {
				...
				if(cAttackerSA == 62 && g_clientList[sTargetH]->m_iEnemyKillCount > g_clientList[sAttackerH]->m_iEnemyKillCount)
				{
					iTemp = (g_clientList[sTargetH]->m_iEnemyKillCount - g_clientList[sAttackerH]->m_iEnemyKillCount) / 100;
					if (iTemp > 7) iTemp = 7;
					iAP_SM += iTemp;
				}
				...
				iTemp = dice(1,10000);
				if ((iTemp >= 1) && (iTemp < 5000))           iHitPoint = 1; // BODY / SHIELD
				...
				switch (iHitPoint) {
				case 1:
					if (g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_LHAND] <= 0 || dice(1,2) == 1) {
						if (g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_BODY] > 0) {
							if (g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_BODY] >= 80) dTmp1 = 80.0f;
							else dTmp1 = (double)g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_BODY];
						
							if (cAttackerType == OWNERTYPE_PLAYER && g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_RHAND] != -1 && g_clientList[sTargetH]->m_pItemList[g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_RHAND]]->m_sIDnum == ITEM_MAGICWAND_MSHIELD) dTmp1 -= (double)g_game->m_pItemConfigList[ITEM_MAGICWAND_MSHIELD]->m_sSpecialEffect;
						...
						if (cAttackerDir != cTargetDir) {
							if (g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_LHAND] >= 80) dTmp1 = 80.0f;
							else dTmp1 = (double)g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_LHAND];
				...
				case 2:
					if ((g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_PANTS] +
						g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_BOOTS]) > 0) {

							if ((g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_PANTS] +
								g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_BOOTS]) >= 80)
								dTmp1 = 80.0f;
							else dTmp1 = (double)(g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_PANTS] + g_clientList[sTargetH]->m_iDamageAbsorption_Armor[EQUIPPOS_BOOTS]);
							dTmp2 = (double)iAP_SM;
							dTmp3 = (dTmp1 / 100.0f) * dTmp2;
				...
				bool shield = false;
				if(cAttackerDir != cTargetDir && g_clientList[sTargetH]->m_iDamageAbsorption_Shield > 0)
				{
					if (dice(1,100) <= (g_clientList[sTargetH]->m_cSkillMastery[SKILL_SHIELD]))
					{
						...
						if (!g_clientList[sTargetH]->IsNeutral() && abs((int)(cTargetDir-cAttackerDir)) == 4)
				...
				if ((cAttackerType == OWNERTYPE_PLAYER) && (g_clientList[sAttackerH] != NULL) && (g_clientList[sAttackerH]->m_bIsSpecialAbilityEnabled == TRUE)) {
					...
					case 0: break;
					case 1:
						/*if (weaponStatus != -1 && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ] != NULL && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ]->m_dwTempItemAttr != 0) iTemp = (g_clientList[sTargetH]->m_iHP / 4);
						else */iTemp = (g_clientList[sTargetH]->m_iHP / 2);
					...
					case 2: 
						if (g_clientList[sTargetH]->m_cMagicEffectStatus[ MAGICTYPE_ICE ] == 0) {
							...
							/*if (weaponStatus != -1 && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ] != NULL && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ]->m_dwTempItemAttr != 0) {
								g_clientList[sTargetH]->m_cMagicEffectStatus[ MAGICTYPE_ICE2 ] = 1;
								g_game->RegisterDelayEvent(DELAYEVENTTYPE_MAGICRELEASE, MAGICTYPE_ICE2, dwTime + (30*1000), 
									sTargetH, OWNERTYPE_PLAYER, NULL, NULL, NULL, 1, NULL, NULL);
								g_game->SendNotifyMsg(NULL, sTargetH, NOTIFY_MAGICEFFECTON, MAGICTYPE_ICE2, 1, NULL, NULL);
							}else {*/
					...
					case 3:
						if (g_clientList[sTargetH]->m_cMagicEffectStatus[ MAGICTYPE_HOLDOBJECT ] == 0) {
							g_game->SendEventToNearClient_TypeB(MSGID_EVENT_COMMON, COMMONTYPE_MAGIC, g_clientList[sAttackerH]->m_cMapIndex, g_clientList[sAttackerH]->m_sX, g_clientList[sAttackerH]->m_sY, g_clientList[sTargetH]->m_sX, g_clientList[sTargetH]->m_sY, NULL, (25+100), g_clientList[sAttackerH]->m_sType);
							/*if (weaponStatus != -1 && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ] != NULL && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ]->m_dwTempItemAttr != 0) {
								if (dice(1,2) == 1) {
								...
								if (g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_NECK] != -1 && g_clientList[sTargetH]->m_pItemList[g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_NECK]]->m_sIDnum == ITEM_NECKLACEOFMEDUSA && dice(1,100) <= 75) {
									g_clientList[sTargetH]->CalcChargeReduct(OBJECTMAGIC, MAGICTYPE_HOLDOBJECT);
									break;
								}

								g_clientList[sTargetH]->m_cMagicEffectStatus[ MAGICTYPE_HOLDOBJECT ] = 1;
								g_game->RegisterDelayEvent(DELAYEVENTTYPE_MAGICRELEASE, MAGICTYPE_HOLDOBJECT, dwTime + (10*1000), 
									sTargetH, OWNERTYPE_PLAYER, NULL, NULL, NULL, 1, NULL, NULL);
								g_game->SendNotifyMsg(NULL, sTargetH, NOTIFY_MAGICEFFECTON, MAGICTYPE_HOLDOBJECT, 1, NULL, NULL);
							//}
						}
						break;

					case 4:
						/*if (weaponStatus != -1 && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ] != NULL && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ]->m_dwTempItemAttr != 0) iAP_SM = g_clientList[sTargetH]->m_iHP/2;
						else */iAP_SM = g_clientList[sTargetH]->m_iHP;
						break;

					case 5:
						/*if (weaponStatus != -1 && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ] != NULL && g_clientList[sAttackerH]->m_pItemList[ weaponStatus ]->m_dwTempItemAttr != 0) g_clientList[sAttackerH]->m_iHP += iAP_SM/2;
						else */g_clientList[sAttackerH]->m_iHP += iAP_SM;
				...
				if ((cAttackerType == OWNERTYPE_PLAYER) && (g_clientList[sAttackerH] != NULL) && (g_clientList[sTargetH]->m_bIsSpecialAbilityEnabled == TRUE)) {
					...
					case 50:
						if (sWeaponIndex != -1 && g_clientList[sAttackerH]->m_pItemList[ sWeaponIndex ]->m_sIDnum != ITEM_GOLDENAXE_LLF)	{
							/*if (g_clientList[sAttackerH]->m_sItemEquipmentStatus[EQUIPPOS_BODY] != -1 && g_clientList[sTargetH]->m_pItemList[ g_clientList[sAttackerH]->m_sItemEquipmentStatus[EQUIPPOS_BODY] ]->m_dwTempItemAttr != 0) iAP_SM *= 0.25;
							else */g_clientList[sAttackerH]->m_pItemList[sWeaponIndex]->m_wCurLifeSpan = 0;
						}
						break;

					case 51:
						if (iHitPoint == g_clientList[sTargetH]->m_iSpecialAbilityEquipPos) {
							/*if (g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_BODY] != -1 &&
								g_clientList[sTargetH]->m_pItemList[ g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_BODY] ]->m_dwTempItemAttr != 0) iAP_SM *= 0.25f;
							else */iAP_SM = 0;
						}
						break;

					case 52:
						if ((sWeaponIndex != -1 && g_clientList[sAttackerH]->m_pItemList[ sWeaponIndex ]->m_sIDnum != ITEM_GOLDENAXE_LLF) || sWeaponIndex == -1)	{
						/*if (g_clientList[sTargetH]->m_pItemList[ g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_LHAND] ]->m_dwTempItemAttr != 0) iAP_SM *= 0.25;
						else */iAP_SM = 0;
						}
						break;
					}
				...
				if (cAttackerDir == cTargetDir && (iHitPoint == 1 || 5) && g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_BACK] != -1) {
					iTemp = g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_BACK];
					EnduStrippingDamage(sTargetH, sAttackerH, cAttackerType, iTemp, 1000, shield);
				}
				...
				case 1:
					...
					EnduStrippingDamage(sTargetH, sAttackerH, cAttackerType, iTemp, 2000, shield);
					break;
				...
				case 2:
					...
					iTemp = g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_BOOTS];
					EnduStrippingDamage(sTargetH, sAttackerH, cAttackerType, iTemp, 2000, shield);
					break;
				...
				case 4:
					...
					EnduStrippingDamage(sTargetH, sAttackerH, cAttackerType, iTemp, 3000, shield);
					break;
				...
				if ((g_clientList[sTargetH]->m_bIsPoisoned == FALSE) && 
					((cAttackerSA == 5) || (cAttackerSA == 6) || ((cAttackerType == OWNERTYPE_PLAYER) && (g_clientList[sAttackerH]->m_iAddPsnDmg > 0))) ) {
						if (checkResistingPoisonSuccess(sTargetH, OWNERTYPE_PLAYER) == FALSE) {
							...
							else if (g_clientList[sAttackerH]->m_iAddPsnDmg > 0) g_clientList[sTargetH]->m_iPoisonLevel = g_clientList[sAttackerH]->m_iAddPsnDmg; 
				...
				if (cAttackerType == OWNERTYPE_PLAYER) {
					if (g_clientList[sAttackerH]->m_iAddLifeLeech > 0) {
						g_clientList[sAttackerH]->m_iHP += iAP_SM*g_clientList[sAttackerH]->m_iAddLifeLeech/100;
						if (g_clientList[sAttackerH]->m_iHP > g_clientList[sAttackerH]->GetMaxHP()) g_clientList[sAttackerH]->m_iHP = g_clientList[sAttackerH]->GetMaxHP();
						g_game->SendNotifyMsg(NULL, sAttackerH, NOTIFY_HP, NULL, NULL, NULL, NULL);
					}
				}

				//if (g_clientList[sTargetH]->m_heroArmourBonus == 2) iAP_SM -= iAP_SM*(7*(((g_clientList[sTargetH]->m_pItemList[g_clientList[sTargetH]->m_sItemEquipmentStatus[EQUIPPOS_HEAD]]->m_dwAttribute & 0xF0000000) >> 28)+1)/3/100);

				if (iAP_SM > 0) {
					if (cAttackerType == OWNERTYPE_PLAYER) g_clientList[sAttackerH]->CalcChargeReduct(OBJECTATTACK);
					g_clientList[sTargetH]->CalcChargeReduct(OBJECTDAMAGE, -1, cAttackerType);
				}
...
CAE_SKIPDAMAGEMOVE:;
						...
						if (g_clientList[sTargetH]->m_cMagicEffectStatus[ MAGICTYPE_HOLDOBJECT ] == 1 && ((cAttackerType == OWNERTYPE_PLAYER && (g_clientList[sAttackerH]->m_iSpecialAbilityType != 3 || !g_clientList[sAttackerH]->m_bIsSpecialAbilityEnabled)) || cAttackerType == OWNERTYPE_NPC)) {
		...
		case OWNERTYPE_NPC:
				...
				if (cAttackerSide == g_npcList[sTargetH]->m_side) {
					...
					case NPC_GMG:
						if (cAttackerSide != 0) {
							...
							if (g_npcList[sTargetH]->m_iV1 > 300) {

								g_npcList[sTargetH]->m_iV1 -= 300;
								g_game->m_mana[g_npcList[sTargetH]->m_side] -= 5;
			...
			if (g_npcList[sTargetH]->m_cSize == 0) iDamage = iAP_SM;
			else iDamage = iAP_L;
			...
			if (cAttackerType == OWNERTYPE_PLAYER) {
				g_clientList[sAttackerH]->m_iHP += iDamage*g_clientList[sAttackerH]->m_iAddLifeLeech/100;
				if (g_clientList[sAttackerH]->m_iHP > g_clientList[sAttackerH]->GetMaxHP()) g_clientList[sAttackerH]->m_iHP = g_clientList[sAttackerH]->GetMaxHP();
				g_game->SendNotifyMsg(NULL, sAttackerH, NOTIFY_HP, NULL, NULL, NULL, NULL);
			}

			if (iDamage > 0 && cAttackerType == OWNERTYPE_PLAYER) g_clientList[sAttackerH]->CalcChargeReduct(OBJECTATTACK);

			g_npcList[sTargetH]->ReduceHP( iDamage );

			if (g_npcList[sTargetH]->m_iHP <= 0) {
				g_npcList[sTargetH]->m_iHP = 0;
...
CAE_SKIPCOUNTERATTACK:;
				...
				if ((wWeaponType < 40 || wWeaponType == 45) && (g_npcList[sTargetH]->m_cActionLimit == 4)) {
...
CAE_SKIPDAMAGEMOVE2:;
		...
		if (cAttackerType == OWNERTYPE_PLAYER) {
			...
			if (sWeaponIndex != -1) {
				...
				{
						iWepLifeOff = 1;
						if (((wWeaponType >= 1) && (wWeaponType < 40)) || wWeaponType == 45) {
							switch (g_mapList[g_clientList[sAttackerH]->m_cMapIndex]->m_cWhetherStatus) {
							case 0:	break;
						...
						if (!g_clientList[sAttackerH]->IsNeutral()) {
							if (g_clientList[sAttackerH]->m_pItemList[sWeaponIndex]->m_wCurLifeSpan < iWepLifeOff) 
								...
								g_clientList[sAttackerH]->m_pItemList[sWeaponIndex]->m_wCurLifeSpan -= iWepLifeOff;
								g_game->SendNotifyMsg(NULL, sAttackerH, NOTIFY_UPDATEITEMLIFESPAN, sWeaponIndex, g_clientList[sAttackerH]->m_pItemList[sWeaponIndex]->m_wCurLifeSpan);
							}
						}
...
void EnduStrippingDamage(short sTargetH, short sAttackerH, char cAttackerType, int item, int higherStripEndu, bool shield)
{
		...
		(g_clientList[sTargetH]->m_side != g_clientList[sAttackerH]->m_side || g_mapList[g_clientList[sAttackerH]->m_cMapIndex]->m_bIsFightZone))
	{
		...
		case SKILL_HAMMER:
			...
			case ITEM_BARBARIANHAMMER:
				...
				break;
			case ITEM_GBHAMMER:
				/*if (g_clientList[sAttackerH]->m_pItemList[equiptItem]->m_dwTempItemAttr != 0) {
					stripChance = 0.52f;
					enduDamage = 25;
				}else {*/
					stripChance = 0.6f;
			...
			if (g_clientList[sTargetH]->m_pItemList[item]->m_cEquipPos == EQUIPPOS_LHAND) stripChance = 0;
			if (g_clientList[sTargetH]->m_pItemList[item]->m_cEquipPos == EQUIPPOS_BACK) return;
			break;
		...
		case SKILL_AXE:
			...
			{
			case ITEM_DOUBLEAXE:
			case ITEM_DOUBLEAXE_ADD1:
			case ITEM_DOUBLEAXE_ADD2:
				stripChance = 0.25;
				enduDamage = 10;
				break;
			case ITEM_WARAXE:
			case ITEM_WARAXE_ADD1:
			case ITEM_WARAXE_ADD2:
			case ITEM_KNIGHTWARAXE:
				stripChance = 0.35f; 
				enduDamage = 15;
				break;
			case ITEM_BLOODAXE:
				...
				break;
			case ITEM_BATTLEAXE:
			case ITEM_BATTLEAXE_ADD1:
			case ITEM_BATTLEAXE_ADD2:
			case ITEM_XELIMAAXE:
			case ITEM_KLONESSAXE:
				stripChance = 0.45f;
				enduDamage = 20;
				break;
			case ITEM_GOLDENAXE_LLF:
				stripChance = 0;
				break;
			}

			if (g_clientList[sTargetH]->m_pItemList[item]->m_cEquipPos != EQUIPPOS_LHAND && g_clientList[sTargetH]->m_pItemList[item]->m_cEquipPos != EQUIPPOS_BACK) return;
			break;  
	...
	if (!g_clientList[sTargetH]->IsNeutral() && 
		...
		g_game->SendNotifyMsg(NULL, sTargetH, NOTIFY_UPDATEITEMLIFESPAN, item, g_clientList[sTargetH]->m_pItemList[item]->m_wCurLifeSpan);
	}
