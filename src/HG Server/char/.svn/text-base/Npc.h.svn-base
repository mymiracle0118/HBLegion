...
public:
	...
	int GetGenLevel();
	virtual void Behavior_Death(Unit * attacker, int16 dmg);
	...
	void AddHP(uint32 value);
	void ReduceHP(uint32 value);

	char  m_cNpcName[21];

	char  m_cName[NPCNAMELENGTH];
	char  m_cMapIndex;
	short m_dX, m_dY;
	short m_vX, m_vY;
	int   m_tmp_iError;
...
typedef std::auto_ptr<CNpc> CNpcPtr;
