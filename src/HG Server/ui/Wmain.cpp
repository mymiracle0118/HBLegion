...
class CItem ** g_itemConfigList = NULL;

class CMisc  *  g_misc = NULL;

int * g_skillSSNpoint = NULL;

HashMap<ItemID, uint8> g_socketMap;

bool g_updateWeb = true;

// DUMMIES
CNpc dummyNpc("DUMMY", -1);
...
void Error(const string str, const char * file, const uint32 line)
{
	...
	sprintf(buffer, "%s(%i)(%d.%d): ERROR:  %s", file, line,
		UPPER_VERSION, LOWER_VERSION, str.c_str()); 
	PutLogFileList((const char*)buffer, ERROR_LOGFILE);
}
//=============================================================================
...
int32 iNdice(int _throw, int range)
{
	...
	for (i = 1; i <= _throw; i++) {
		...
		if (range > RAND_MAX) ret += ((int)(((float)rand() / RAND_MAX) * range)) + 1;
		else ret += (rand() % range) + 1;
	}
...
void parseCommand(char* pMsg)
{
	...
	*/
		...
		{
			if(g_clientList[i]) {
				wsprintf(buff, "Char: %s(%d)(%s) on %s (%s)  Guild: %s(%i)", g_clientList[i]->m_cCharName, 
					g_clientList[i]->m_iLevel,	sideName[ g_clientList[i]->m_side] ,
					g_clientList[i]->m_cAccountName,	g_clientList[i]->m_cIPaddress,
					g_clientList[i]->m_cGuildName, g_clientList[i]->m_iGuildGUID);
				PutLogList(buff);
			}
		}
	}else if(memcmp(pMsg, "guildlist", 9) == 0){
		while(CGuild * guild = g_game->m_guildMgr.Get())
		{
			wsprintf(buff, "Guild: %s, %i (%u online)", guild->m_name, guild->m_guildGUID, guild->GetOnlineCnt());
			PutLogList(buff);
		}
	}else if(memcmp(pMsg, "websiteupdate", 13) == 0){
		g_updateWeb = !g_updateWeb;
	}else{
