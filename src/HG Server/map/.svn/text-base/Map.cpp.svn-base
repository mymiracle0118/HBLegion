...
Unit ** CMap::GetOwners(short x1, short x2, short y1, short y2)
{
	class CTile * pTile;
	...
	for(int y = y1; y <= y2; y++) 
	{
		for (int x = x1; x <= x2; x++)
		{
			pTile = (class CTile *)(m_pTile + x + y*m_sSizeY);
...
bool CMap::bGetMoveable(short dX, short dY, short * pDOtype, short sMoverID, class CItem * pTopItem) 
{
	class CTile * pTile;
	...
	if (pTile->m_sOwner != NULL && sMoverID == NULL) return FALSE;
	if (pTile->m_sOwner != NULL && sMoverID != pTile->m_sOwner) return FALSE;
...
bool CMap::bSetItem(short sX, short sY, class CItem * pItem)
{
	...
	if (pItem->m_cTouchEffectType == ITET_UNIQUE_OWNER) return FALSE;
...
class CItem * CMap::pGetItem(short sX, short sY, short * pRemainIDnum, char * pRemainItemColor, DWORD * pRemainItemAttribute)
{
	class CTile * pTile;
	class CItem * pItem;
	int i;
	...
	if (pTile->m_pItem[0] == NULL) {
		*pRemainIDnum = 0;
		*pRemainItemAttribute = 0;
		*pRemainItemColor = 0;
	}
	else
	{
		*pRemainIDnum = pTile->m_pItem[0]->m_sIDnum;
		*pRemainItemAttribute = pTile->m_pItem[0]->m_dwAttribute;
...
int CMap::GetEmptyNamingValue(char * pName, int map)
{
	int i;

	for (i = 0; i < 2000; i++){
		if (m_bNamingValueUsingStatus[i] == FALSE) {
			ZeroMemory(pName, NPCNAMELENGTH);
			wsprintf(pName, "_%c%d", map + 65, i);
