...
{
    inline bool operator() (const guildsx& struct1, const guildsx& struct2)
    {
	...
	void RequestPlayerTitle(int iClientH,int iObjectID);
	void ChangeTitleHandler(int iClientH,char*pData,DWORD dwMsgSize);
	int AddTitleToPlayer(int iClientH,int iTitle,int iUses);
	int m_iBisleToggle;
	string m_cBisleGuild[5];
	int m_iBisleGuild[5];
	bool m_bBisleEvent;
	void ProcessBisleScores();
	void ProccessBisleWinner(int iAttackerH);
	...
	void ReqAddAuctionItem(int iClientH, char *pData);
	void RequestPingAlert(int iClientH, char * pData, DWORD dwMsgSize);
	void HeldenianPlayerKill(CClient * killer, CClient * victim);
	void BisleEventToggle(bool bActive);
	...
	void ValidateEquipment(int iClientH);
	...
	void AdminOrder_ResetSpecialTime(int iClientH);
	...
	void Effect_Damage_Spot_DamageMove(short sAttackerH, char cAttackerType, short sTargetH, char cTargetType, short sAtkX, short sAtkY, short sV1, short sV2, short sV3, bool bExp, int iAttr, CMagic * spell = NULL);
	...
	uint32 RollMultiple(CNpc * npc, int iItemSpreadType, int iSpreadRange, int *iItemIDs, short *sTempAttrs, short *sTempCharges, Point *BasePos);
	...
	void ReqRepairItemCofirmHandler(int iClientH, char cItemID, short sRepairCharge, char * pString);
	void ReqRepairItemHandler(int iClientH, char cItemID, char cRepairWhom, short sRepairCharge, char * pString);
	...
	bool bSetItemToBankItem(int iClientH, CItem * pItem, BOOL bNationChange = FALSE);
	...
	void Effect_Damage_Spot(short sAttackerH, char cAttackerType, short sTargetH, char cTargetType, short sV1, short sV2, short sV3, bool bExp, int iAttr = NULL, CMagic * spell = NULL);
	...
	void SetSkillAll(int iClientH);//acidx remove
	...
	void RequestChargeMagicHandler(int iClientH, char * pData);
	bool RequestStudyMagicHandler(int iClientH, char * pName, DWORD dwTempItemAttr, bool bIsPurchase = TRUE);
	...
	BOOL PlayerMagicHandler(int iClientH, int dX, int dY, short sType, bool bItemEffect = FALSE, int iV1 = NULL, bool bNoManaCost = FALSE);
	...
	void TradeCont(int iClientH, char * pData, DWORD dwMsgSize);
	void GetSlayerItemHandler(int iClientH, char * pData, DWORD dwMsgSize);
	void UpgradeHeroSetHandler(int iClientH, char * pData, DWORD dwMsgSize);
	...
	void SendNotifyMsg(int iFromH, int iToH, WORD wMsgType, DWORD sV1 = NULL, DWORD sV2 = NULL, DWORD sV3 = NULL, const char * pString = NULL, 
	...
	void ReqAuctionItemList(int iClientH, char * pData);
	void ResponseAuctionItemList(char * pData, DWORD dwMsgSize);
	...
	char cGetNextMoveDir(short sX, short sY, short dstX, short dstY, char cMapIndex, char cTurn, int * pError, char cSize = 1, short sMoverID = NULL);
	char cGetNextMoveDir(short sX, short sY, short dstX, short dstY, char cMapIndex, char cTurn, int * pError, short * DOType, char cSize = 1, short sMoverID = NULL);
	...
	void NpcProcess();
	bool bCreateNewNpc(char * pNpcName, char * pName, int mapIndex, char cSA, char cMoveType, int * poX, int * poY, char * pWaypointList, RECT * pArea, int iSpotMobIndex, char cChangeSide, bool bHideGenMode, bool bIsSummoned = FALSE, bool bFirmBerserk = FALSE, bool bIsMaster = FALSE, int iGuildGUID = NULL);
	//bool bCreateNewNpc(char * pNpcName, char * pName, char * pMapName, short sX, short sY);
	bool _bReadMapInfoFiles(int iMapIndex);
	void UpdateAllClientsOfBisleEnemies();
	...
	bool bSendMsgToLS(DWORD dwMsg, int iClientH, bool bFlag = TRUE,char * pData = NULL, uint32 length = 0 );
	...
	void SendEventToNearClient_TypeB(DWORD dwMsgID, WORD wMsgType, char cMapIndex, short sX, short sY, short sV1 = 0, short sV2 = 0, const char * pString = NULL, int sV3 = 0, int sV4 = 0);
	...
	void RequestQuestListHandler(int iClientH, char * pData, DWORD dwMsgSize);
	void RequestAcceptQuestHandler(int iClientH, char * pData, DWORD dwMsgSize);
	...
	void StatChangeHandler(int iClientH, char * pData, DWORD dwMsgSize);
	void CheckMagicInt(int iClientH, bool bDisable = false);
	...
	void UpdateName(int clientH, char *NewName);
	void StatReset(int clientH);
	bool ChangeNation(int clientH, Side side);
	void LegionCash(char * account, char type, char i, bool Type);
	bool GetLegionItemCount(int clientH, char * cItemName, long count);
	bool GetLegionItem(int clientH, WORD cmd, char sex = 0);
	bool GetLegionPoints(int clientH, DWORD price);
	bool GetSpellBack(int clientH, WORD cmd);
...
//private:
	...
	CGuildMgr m_guildMgr;
	CPartyMgr m_partyMgr;
	...
	bool  m_bF1pressed, m_bF4pressed, m_bF5pressed, m_bF12pressed;
	bool  m_bOnExitProcess;
	DWORD m_dwExitProcessTime;
	DWORD m_dwEventScheduleTimer;
